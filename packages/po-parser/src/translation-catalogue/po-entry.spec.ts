...
describe('POT entries', () => {
	describe('simple cases', () => {
		it('should output singular entries', () => {
			const entry = new PoEntry({ msgid: 'foobar' });
...
`;
		...
		it('should output plural entries', () => {
			const entry = new PoEntry({ msgid: 'foobar', msgidPlural: 'foobars' });
			const expected = `msgid "foobar"
...
`;
		...
		it('should output message contexts', () => {
			const entry = new PoEntry({ msgid: 'Open', msgctxt: 'Menu' });
...
`;
	...
	describe('escaping', () => {
		...
		it('should escape a backspace character', () => {
			const entry = new PoEntry({ msgid: '\b' });
...
`;
		...
		it('should escape a vertical tab', () => {
			const entry = new PoEntry({ msgid: '\v' });
			const expected = `msgid "\\v"\nmsgstr ""\n`;
			expect(entry.toString()).toEqual(expected);
		});
		it('should escape a form feed', () => {
			const entry = new PoEntry({ msgid: '\f' });
			const expected = `msgid "\\f"\nmsgstr ""\n`;
			expect(entry.toString()).toEqual(expected);
		});
		it('should escape a carriage return', () => {
			const entry = new PoEntry({ msgid: '\r' });
			const expected = `msgid "\\r"\nmsgstr ""\n`;
			expect(entry.toString()).toEqual(expected);
		});
		it('should throw an exception for all other controls', () => {
			for (let i = 0; i < 0x7; ++i) {
				expect(() => new PoEntry({ msgid: String.fromCharCode(i) })).toThrow();
				expect(
					() =>
						new PoEntry({ msgid: 'x', msgidPlural: String.fromCharCode(i) }),
				).toThrow();
			}
	...
	describe('line wrapping', () => {
		it('should wrap long lines by default', () => {
			const entry = new PoEntry({ msgid: 'For a very long time.' });
			const expected = `msgid ""
"For a very long "
"time."
msgstr ""
...
`;
		...
		it('should always wrap at newline characters', () => {
			const entry = new PoEntry({ msgid: 'line 1\nline 2\nline 3\n' });
			const expected = `msgid ""
"line 1\\n"
"line 2\\n"
"line 3\\n"
...
`;
		...
		it('should never wrap inside words', () => {
			const entry = new PoEntry({ msgid: 'ForAVeryLongTime' });
			const expected = `msgid ""
"ForAVeryLongTime"
msgstr ""
`;
		...
		it('should never wrap inside words with prefix and postfix', () => {
			const entry = new PoEntry({ msgid: 'before ForAVeryLongTime after' });
			const expected = `msgid ""
"before "
"ForAVeryLongTime "
"after"
...
`;
		...
		it('should not exceed the requested width', () => {
			const entry = new PoEntry({ msgid: 'At the Height of the Fighting' });
			const expected = `msgid ""
"At the Height "
"of the "
"Fighting"
...
`;
	...
	describe('merge entries', () => {
		const entry = new PoEntry({ msgid: 'hello' });
...
`;
	...
	describe('errors', () => {
		it('should reject file names with spaces', () => {
			...
			expect(() => new PoEntry({ msgid, references })).toThrow();
		});
		...
		it('should reject flags with commas', () => {
			...
			expect(() => new PoEntry({ msgid, flags })).toThrow();
		});
	});
});
