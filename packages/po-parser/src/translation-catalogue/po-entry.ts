...
export class PoEntry {
	...
	static build(): PoEntry {
		const entry = new PoEntry({ msgid: 'ignore' });
	...
	toString(width = 76): string {
		...
		if (typeof this.properties.msgidPlural === 'undefined') {
			...
			// FIXME! This must be modified for entries with translations.
			out +=
				this.serializeMsgId(
	 ...
	 */
	public merge(other: PoEntry): void {
	 ...
	 */
	addToMsgstr(chunk: string, index: number = 0): void {
		if (!this.properties.msgstr) {
			this.properties.msgstr = [];
		}
		if (typeof this.properties.msgstr[index] === 'undefined') {
			this.properties.msgstr[index] = '';
		}

		this.properties.msgstr[index] += chunk;
	}
